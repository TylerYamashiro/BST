...
class BST
{
 ...
 public:
  ...
  int getCountInRange(BinNode* root, int low, int high);
  //Precondition: A BST exists
  //PostCondition: Returns the number of items int eh range using recursion
  int getCount(int low, int high);
  //Precondiiton: A BST exists
  //PostCondition: Callse getCountInRange() to recursivly determine the number
  //of items in the range
  int findMinValue();
  //PreCondition: A BST exists
  //PostCondition: Calls findMinHelper() to recursivly find minimum 
  int findMinHelper(BinNode* root);
  //PreCondition: A BST exists
  //PostCondition: Recursively returns the minimum of a BST
  BinNode* find(BinNode* root, int item);
  //PreCondition: A BST exits
  //PostConditon: Returns a Node with the spacified number
  int returnPredecessor(int key);
  //PreCondition: A BST exists
  //Postcondition: Calls findPrecessor() to recursivley find the predecessor
  //of the key specivied
  int findPredecessor(BinNode* root, int key);
  //PreCondition: A BST exists
  //PostCondition: Recursivly finds the Predecessor of the given key
  int lca(BinNode* root, int n1, int n2);
  //PreCondition: A BST exists
  //PostCondition: recursively finds the lca between two items
  int getLCA(int n1, int n2);
  //Preconditoon: A BST exists
  //PostCondition: recursively calls lca() to find the lca between two
  //items specified by parameters
  void arrayInOrder(BinNode* root, int*& arr, int& index);
  //PreConditon: A BST exists
  //PostCondition: Creates an array from the BST in order recursively
  void returnBSTSize(BinNode*, int& size);
  //PreCondition: A BST exists
  //PostCondition: recursivly finds the size of a bst and passes it by
  //reference
  int findPairWithSum(BinNode* root, int targetSum);
  //PreConditon: A BST exists
  //PostConditon:Recursively tests a BST to see if a specified sum can be
  //created from two items in it
  int pairWithSum(int targetSum);
  //PreConditon: A BST exists
  //PostConditon: Calls findPairWithSum() to recursivly find if there is a sum
  //if there are two items whos sum is the taget item
  int returnSecondLargest();
  //PreCondition: A BST exists
  //PostConditio: calls secondLargesElement() to return the second larges
  //element in a BST recursivley
  void secondLargestElement(BinNode* root, int& counter, int& value);
  //Preconditon: A BST exists
  //PostConditon: recursivly finds the second larges element and passes
  //the value by reference to returnSecondLargest();
  void printCommon(BinNode* root1, BinNode* root2);
  //Preconditon:A BST exists
  //PostCondition: Recursivly checks if there are common nodes between trees
  //by turning them into arrays and comparing
  void printCommonNodes(BinNode* root2);
  //PreConditon: A BST exists
  //PostCondition: Calls printCommon() to recursivly print the nodes two trees
  //have in common
  BinNode* returnRoot();
  //Preconditon: A BST exists
  //PostCodntion: returns the root of the tree
};
